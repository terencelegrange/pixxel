...
const EMPTY: StrategyForm = { name: "", description: "", sortOrder: "" };

function strategyToForm(s: AssetStrategy): StrategyForm {
  return {
    name: s.name,
...
function StrategyModal({
  ...
  isOpen: boolean;
  onClose: () => void;
  editing: AssetStrategy | null;
  onSave: (form: StrategyForm) => Promise<void>;
}) {
  const [form, setForm] = useState<StrategyForm>(EMPTY);
  ...
  useEffect(() => {
    if (isOpen) {
      setForm(editing ? strategyToForm(editing) : EMPTY);
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) { setNameError("Strategy name is required."); return; }
    setNameError(""); setGeneralError("");
    setIsSaving(true);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={editing ? `Edit — ${editing.name}` : "Add Strategy"}
      maxWidth="max-w-lg"
    >
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex gap-4">
            <div className="flex-1">
              <Input
                label="Strategy name"
                type="text"
                placeholder="e.g. Emerging, Adopting, Productionised"
                value={form.name}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              placeholder="Optional description of what this strategy classification means…"
        ...
        <div className="mt-6 flex justify-end gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
          ...
          <Button type="submit" isLoading={isSaving}>
            {editing ? "Save changes" : "Add Strategy"}
          </Button>
        </div>
      </form>
    </Modal>
...
export default function AssetStrategyPage() {
  ...
  const [editing, setEditing] = useState<AssetStrategy | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<AssetStrategy | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/asset-strategy");
  ...
  async function handleSave(form: StrategyForm) {
    if (!user) return;
    const url    = editing ? `/api/asset-strategy/${editing.id}` : "/api/asset-strategy";
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/asset-strategy/${deleteTarget.id}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
              <thead className="bg-slate-50 dark:bg-slate-800/50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Strategy</th>
                  ...
                  <th className="hidden px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 w-24 md:table-cell">Order</th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900">
                {filtered.map((strategy) => (
                  <tr key={strategy.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-slate-100">
                          <Target className="h-4 w-4 text-slate-500" />
                        </div>
                        <p className="font-medium text-slate-900 dark:text-slate-100">{strategy.name}</p>
                      </div>
                    </td>
                    <td className="hidden px-6 py-4 text-sm text-slate-500 sm:table-cell max-w-sm">
                      {strategy.description
                        ? <span className="line-clamp-2">{strategy.description}</span>
                        : <span className="italic text-slate-300">—</span>
                      }
                    </td>
                    <td className="hidden px-6 py-4 text-sm text-slate-500 md:table-cell w-24">
                      {strategy.sortOrder != null
                        ? <span className="font-mono">{strategy.sortOrder}</span>
                        : <span className="italic text-slate-300">—</span>
                      }
                    </td>
      ...
      {/* Footer count */}
      {!isLoading && !fetchError && strategies.length > 0 && (
        <p className="text-xs text-slate-400">
          {filtered.length} of {strategies.length} strateg{strategies.length !== 1 ? "ies" : "y"}
        </p>
      )}
      ...
      {/* Delete Confirmation */}
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Strategy" maxWidth="max-w-md">
        <div className="flex flex-col gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-slate-700 dark:text-slate-300">
                ...
                Any assets assigned to this strategy will be unassigned.
              </p>
