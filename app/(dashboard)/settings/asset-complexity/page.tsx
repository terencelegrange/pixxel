...
export default function AssetComplexityPage() {
  ...
  const [editing, setEditing] = useState<AssetComplexity | null>(null);
  const [form, setForm] = useState<FormState>(EMPTY);
  const [formError, setFormError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const [deleteTarget, setDeleteTarget] = useState<AssetComplexity | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const res = await fetch("/api/asset-complexity");
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to load.");
      setItems(data.complexities);
    } catch (err) {
  ...
  function openEdit(item: AssetComplexity) {
    setEditing(item);
    setForm({
      name: item.name,
  ...
  async function handleSave() {
    ...
    setFormError(null);
    try {
      const url = editing ? `/api/asset-complexity/${editing.id}` : "/api/asset-complexity";
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/asset-complexity/${deleteTarget.id}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Asset Complexity</h1>
      ...
      {!isLoading && !fetchError && items.length > 0 && (
        <p className="text-xs text-slate-400">{items.length} level{items.length !== 1 ? "s" : ""}</p>
      )}
      ...
      <Modal
        ...
        title={editing ? `Edit — ${editing.name}` : "Add Complexity Level"}
        maxWidth="max-w-md"
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
            ...
            <Button isLoading={isSaving} onClick={handleSave}>
              {editing ? "Save changes" : "Add Complexity"}
            </Button>
          </div>
        </div>
      </Modal>
