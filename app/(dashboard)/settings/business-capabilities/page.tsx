  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 py-20 text-slate-400 dark:text-slate-500 rounded-xl border border-slate-200 bg-white shadow-sm dark:bg-slate-900 dark:border-slate-700">
        ...
        <div className="space-y-4">
          {grouped.map(({ sector, caps }) => {
            ...
            return (
              <div key={sector.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-700">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-5 py-3">
                  ...
                  >
                    <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">{sector.name}</h2>
                    ...
                    {isCollapsed
                      ? <ChevronDown className="h-4 w-4 text-slate-400 dark:text-slate-500 ml-auto" />
                      : <ChevronUp className="h-4 w-4 text-slate-400 dark:text-slate-500 ml-auto" />
                    }
                  </button>
                  {canWrite && (
                    <button
                      onClick={() => openCreate(sector.id)}
                      className="ml-4 flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-medium text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-950/50 transition-colors"
                    >
                ...
                {!isCollapsed && (
                  <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                    <thead className="bg-white dark:bg-slate-900">
                      <tr>
                        <th className="px-6 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Capability</th>
                        <th className="hidden px-6 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 md:table-cell">Description</th>
                        <th className="hidden px-6 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 sm:table-cell w-20">Order</th>
                        <th className="px-6 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                      {caps.map((c) => (
                        <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                          <td className="px-6 py-3 font-medium text-slate-900 dark:text-slate-100 text-sm">{c.name}</td>
                          <td className="hidden px-6 py-3 text-sm text-slate-500 dark:text-slate-400 md:table-cell">
                            {c.description || <span className="italic text-slate-300 dark:text-slate-600">—</span>}
                          </td>
                          <td className="hidden px-6 py-3 text-sm text-slate-400 dark:text-slate-500 sm:table-cell">
                            {c.sortOrder ?? <span className="italic text-slate-300 dark:text-slate-600">—</span>}
                          </td>
                          <td className="px-6 py-3">
                            <div className="flex items-center justify-end gap-2">
                              {canWrite && (
                                <>
                                  <button onClick={() => openEdit(c)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-300 transition-colors" aria-label="Edit">
                                    <Pencil className="h-4 w-4" />
                                  </button>
                                  <button onClick={() => { setDeleteTarget(c); setDeleteError(null); }} className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/50 dark:hover:text-red-400 transition-colors" aria-label="Delete">
          ...
          {ungrouped.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-700">
              <div className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-5 py-3">
                <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400 italic">Unassigned</h2>
              </div>
              <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                  {ungrouped.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                      <td className="px-6 py-3 font-medium text-slate-900 dark:text-slate-100 text-sm">{c.name}</td>
                      <td className="hidden px-6 py-3 text-sm text-slate-500 dark:text-slate-400 md:table-cell">{c.description || <span className="italic text-slate-300 dark:text-slate-600">—</span>}</td>
                      <td className="px-6 py-3">
                        <div className="flex items-center justify-end gap-2">
                          {canWrite && (
                            <>
                              <button onClick={() => openEdit(c)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-300 transition-colors"><Pencil className="h-4 w-4" /></button>
                              <button onClick={() => { setDeleteTarget(c); setDeleteError(null); }} className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/50 dark:hover:text-red-400 transition-colors"><Trash2 className="h-4 w-4" /></button>
                            </>
                          )}
                        </div>
                      </td>
      ...
      {!isLoading && !fetchError && totalCount > 0 && (
        <p className="text-xs text-slate-400 dark:text-slate-500">{totalCount} capability{totalCount !== 1 ? "s" : ""} across {sectors.filter(s => capabilities.some(c => c.industrySectorId === s.id)).length} sector{sectors.length !== 1 ? "s" : ""}</p>
      )}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? `Edit — ${editing.name}` : "Add Business Capability"} maxWidth="max-w-md">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Industry sector<span className="ml-0.5 text-red-500">*</span></label>
            <select value={form.industrySectorId} onChange={(e) => setForm((f) => ({ ...f, industrySectorId: e.target.value }))} className="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-1 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100">
