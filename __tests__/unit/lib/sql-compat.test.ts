...
describe('insertIgnoreSql', () => {
  it('produces MySQL INSERT IGNORE syntax with quoted columns', () => {
  ...
  it('produces SQLite INSERT OR IGNORE syntax with quoted columns', () => {
...
describe('upsertSql', () => {
  // `key` is a reserved word in MySQL/MariaDB — every column must be
  // backtick-quoted or this breaks with a SQL syntax error (as it did in
  // production against app_settings(key, value) before this was fixed).
  it('produces MySQL ON DUPLICATE KEY UPDATE syntax with quoted columns', () => {
  ...
  it('produces SQLite ON CONFLICT DO UPDATE syntax with quoted columns, conflicting on the first column', () => {
