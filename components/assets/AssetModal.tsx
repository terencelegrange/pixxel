  ...
  return (
    <div className="col-span-2 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium text-slate-700">Asset icon</label>
        <span className="flex items-center gap-1.5 text-xs text-slate-400">
          <AssetIcon name={value} className="h-3.5 w-3.5" />
          {value}
        </span>
      </div>
      <div className="grid grid-cols-8 gap-1 rounded-lg border border-slate-200 bg-slate-50 p-2 max-h-48 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-4">
          ...
          <div className="col-span-2">
            <div className="flex flex-col gap-1.5">
              ...
              <div className={[
                "max-h-40 overflow-y-auto rounded-lg border bg-white p-2",
                errors.departmentIds ? "border-red-400" : "border-slate-300",
              ].join(" ")}>
          ...
          <div className="col-span-2">
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-slate-700">Architect Responsible</label>
              ...
              <div className="max-h-32 overflow-y-auto rounded-lg border border-slate-300 bg-white p-2">
                {users.length === 0 ? (
                  ...
                  users.map((u) => {
                    ...
                    return (
                      <label key={u.id} className="flex cursor-pointer items-center gap-2.5 rounded-md px-2 py-1.5 hover:bg-slate-50">
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => {
          ...
          <div className="col-span-2">
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-slate-700">Business Capabilities</label>
                {form.capabilityIds.length > 0 && (
                  <span className="text-xs text-slate-400">{form.capabilityIds.length} selected</span>
                )}
              </div>
              <div className="max-h-48 overflow-y-auto rounded-lg border border-slate-300 bg-white p-2">
                    ...
                    return (
                      <div key={sector.id} className="mb-2 last:mb-0">
                        ...
                        {sectorCaps.map((cap) => {
                          ...
                          return (
                            <label key={cap.id} className="flex cursor-pointer items-center gap-2.5 rounded-md px-2 py-1.5 hover:bg-slate-50">
                              <input
                                type="checkbox"
                                checked={checked}
                                onChange={() => {
